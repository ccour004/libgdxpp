...
class MipMapGenerator {
	static void generateMipMapGLES20 (int target, SDL2::Surface data,int format) {
		glTexImage2D(target, 0, format, data->w, data->h, 0,
			format, GL_UNSIGNED_BYTE, data->pixels);
		glGenerateMipmap(target);
	}

	static void generateMipMapDesktop (int target, SDL2::Surface data, int textureWidth, int textureHeight,int format) {
	...
	static void generateMipMapCPU (int target, SDL2::Surface data, int textureWidth, int textureHeight,int format) {
		glTexImage2D(target, 0, format, data->w, data->h, 0,format, GL_UNSIGNED_BYTE, data->pixels);
...
public:
    static void generateMipMap (SDL2::Surface data, int textureWidth, int textureHeight) {
	...
	static void generateMipMap (SDL2::Surface data, int textureWidth, int textureHeight,bool useHWMipMap) {
		generateMipMap(GL_TEXTURE_2D, data, textureWidth, textureHeight,useHWMipMap);
	}
    
    static void generateMipMap (int target, SDL2::Surface data, int textureWidth, int textureHeight){
        generateMipMap(target,data,textureWidth,textureHeight,true);
    }

	/** Sets the image data of the {@link Texture} based on the {@link Pixmap}. The texture must be bound for this to work. If
	 * <code>disposePixmap</code> is true, the pixmap will be disposed at the end of the method. */
	static void generateMipMap (int target, SDL2::Surface data, int textureWidth, int textureHeight,bool useHWMipMap) {
